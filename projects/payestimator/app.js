...
module.exports.projectYear = projectYear;
async function projectYear(num_months = 12) {
  ...
  for (var i = 0; i < num_months; i++) {
    ...
    var month_schedule = getScheduleForMonth(month + 1, year);
    var hours =
      getHours(
        month + 1,
        month_schedule.daysIn,
        year,
        month_schedule.avg_hours_in || 7
      ) -
      avg_days_missed * (month_schedule.avg_hours_in || 7);
  ...
  if (require.main == module) console.table(table);
